...
class FrameExtractor(object):
    ...
    def extract_keyframe_indices(self, videopath, num_keyframes):
        """
        Extract indices of keyframes based on frame differences.

        :param videopath: Path to the video file.
        :type videopath: str
        :param num_keyframes: Number of desired keyframes.
        :type num_keyframes: int
        :return: List of keyframe indices.
        :rtype: list[int]
        """
        # Get all frames and differences
        frame_extractor_generator = self.__extract_all_frames_from_video__(videopath)
        frame_diffs = []
        all_frames = []

        for frames, diffs in frame_extractor_generator:
            all_frames.extend(frames)
            frame_diffs.extend(diffs)

        # Normalize differences and get top `num_keyframes` indices
        diff_array = np.array(frame_diffs)
        keyframe_indices = np.argsort(diff_array)[-num_keyframes:]
...
if __name__ == "__main__":
    ...
    parser.add_argument("num_keyframes", type=int, help="Number of desired keyframes.")
    args = parser.parse_args()

    # Ensure the video file exists
    if not os.path.exists(args.video_path):
        raise FileNotFoundError(f"Video file {args.video_path} not found.")

    # Initialize the FrameExtractor and process the video
    extractor = FrameExtractor()
    keyframe_indices = extractor.extract_keyframe_indices(args.video_path, args.num_keyframes)

    # Output the indices of the keyframes
