...
class Video(object):
    ...
    def _extract_keyframes_from_video(self, no_of_frames, file_path):
        """Core method to extract keyframe indices for a video

        :param no_of_frames: Number of keyframes to extract
        :type no_of_frames: int
        :param file_path: Path to the video file
        :type file_path: str
        :return: List of indices of the selected keyframes
        :rtype: list
        """
        ...
        # Passing all the clipped videos for the frame extraction using the map function of the
        ...
        # Convert the nested list of extracted frames into a 1D list
        extracted_candidate_frames = functools.reduce(operator.iconcat, extracted_candidate_frames, [])

        # Get the indices of extracted candidate frames
        extracted_candidate_indices = list(range(len(extracted_candidate_frames)))

        self._remove_clips(chunked_videos)
        image_selector = ImageSelector(self.n_processes)

        # Select the indices of the top keyframes instead of the frames themselves
        top_frame_indices = image_selector.select_best_frame_indices(
            extracted_candidate_indices, no_of_frames
        )

        del extracted_candidate_frames

        return top_frame_indices
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract keyframe indices from a video.")
    parser.add_argument(
        "file_path", type=str, help="Path to the video file"
    )
    parser.add_argument(
        "no_of_frames", type=int, help="Number of keyframes to extract"
    )
    args = parser.parse_args()

    # Instantiate the necessary classes (adjust as per your implementation)
    video_processor = Video()  # Replace with your class name
    try:
        keyframe_indices = video_processor._extract_keyframes_from_video(
            args.no_of_frames, args.file_path
        )
